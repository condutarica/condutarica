...
const TIPOS_INV_USD=[{v:"stock",l:"Stock (Ação EUA)"},{v:"etf_us",l:"ETF EUA"},{v:"reit",l:"REIT"},{v:"cripto",l:"Criptoativo"},{v:"bdr",l:"BDR"},{v:"outro",l:"Outro"}];
const TIPOS_INV=[{v:"acao",l:"Ação BR"},{v:"fii",l:"FII"},{v:"etf",l:"ETF BR"},{v:"outro",l:"Outro"}];
...
const mesAnoDeData=s=>{if(!s)return"";const p=s.split("/");return p.length===3?p[1].padStart(2,"0")+"/"+p[2]:"";}
...
function calcSaldoMes(movs,rendimentos,mesAno){
  const e=(movs||[]).filter(m=>m.tipo==="entrada"&&mesAnoDeData(m.data)===mesAno).reduce((a,m)=>a+(m.valor||0),0);
  const r=(rendimentos||[]).filter(r=>mesAnoDeData(r.data)===mesAno).reduce((a,r)=>a+(r.valor||0),0);
  const s=(movs||[]).filter(m=>m.tipo==="saida"&&mesAnoDeData(m.data)===mesAno).reduce((a,m)=>a+(m.valor||0),0);
  return e+r-s;
}
function calcSaldoMesComInv(movs,rendimentos,ativos,rf,ativos_usd,cotacao_dolar,mesAno){
  const base=calcSaldoMes(movs,rendimentos,mesAno);
  const aporteRV=(ativos||[]).flatMap(a=>(a.aportes||[]).filter(ap=>mesAnoDeData(ap.data)===mesAno)).reduce((a,ap)=>a+(ap.qtd||0)*(ap.preco||0),0);
  const aporteRF=(rf||[]).filter(r=>mesAnoDeData(r.data)===mesAno).reduce((a,r)=>a+(r.valor||0),0);
  const aporteUSD=(ativos_usd||[]).flatMap(a=>(a.aportes||[]).filter(ap=>mesAnoDeData(ap.data)===mesAno)).reduce((a,ap)=>a+(ap.qtd||0)*(ap.preco||0)*(cotacao_dolar||0),0);
  return base-aporteRV-aporteRF-aporteUSD;
}
...
function calcCarteira(ativos){
  ...
  return ativos.map(a=>{
    ...
    }catch(_){return {...a,totalCotas:0,totalInvestido:0,precoMedio:0,valorAtual:0,lucro:0,pct:0,totalRecebido:0};}
  });
}
...
const CONQUISTAS=[
  ...
  {id:"primeiro_investimento",icone:"📈",nome:"Investidor Iniciante",desc:"Fez o primeiro aporte",pts:100,check:d=>(d.ativos||[]).some(a=>(a.aportes||[]).length>0)||(d.rf||[]).length>0||(d.ativos_usd||[]).some(a=>(a.aportes||[]).length>0)},
...
function calcPontos(d){let pts=0;CONQUISTAS.forEach(c=>{try{if(c.check(d))pts+=c.pts;}catch(_){}});return pts;}
function calcNivel(pts){let n=NIVEIS[0];NIVEIS.forEach(x=>{if(pts>=x.min)n=x;});return n;}
function calcConquistas(d){return CONQUISTAS.map(c=>{try{return{...c,desbloqueada:c.check(d)};}catch(_){return{...c,desbloqueada:false};}});}
function atualizarSequencia(d){
  ...
  if(d.ultimo_acesso_dia===hoje2)return null;
  if(d.ultimo_acesso_dia===ontemStr)return{sequencia_dias:(d.sequencia_dias||0)+1,ultimo_acesso_dia:hoje2};
...
function exportarPDF(d,modulos){
  ...
  if(modulos.financeiro){
    ...
    meses.forEach(ma=>{
      ...
      if(movMes.length>0){html+=`<table><tr><th>Tipo</th><th>Descrição</th><th>Valor</th><th>Categoria</th><th>Emoção</th></tr>`;movMes.forEach(m=>{html+=`<tr><td>${m.tipo}</td><td>${m.descricao}</td><td>${m.tipo==="saida"?"-":""}${fmt(m.valor)}</td><td>${m.categoria}</td><td>${m.emocao||"-"}</td></tr>`;});html+=`</table>`;}
    });
  }
  if(modulos.emocional&&todasEmo.length>0){html+=`<h2>😊 Mapa Emocional</h2>`;const freq={};todasEmo.forEach(e=>{freq[e.nome]=(freq[e.nome]||0)+1;});html+=Object.entries(freq).map(([n,q])=>`<span class="badge ${EP.includes(n)?"pos":"neg"}">${n} (${q})</span> `).join("");}
  if(modulos.crencas&&crencas.length>0){html+=`<h2>🧠 Crenças</h2><table><tr><th>Crença</th><th>Status</th><th>Nota</th></tr>`;crencas.forEach(c=>{html+=`<tr><td>${c.crenca}</td><td><span class="badge ${c.status==="ressignificada"?"pos":"neg"}">${c.status==="ressignificada"?"Ressignificada":"Ativa"}</span></td><td>${c.notaMentora||"-"}</td></tr>`;});html+=`</table>`;}
  if(modulos.metas&&metas.length>0){html+=`<h2>🎯 Metas</h2><table><tr><th>Meta</th><th>Tipo</th><th>Progresso</th><th>Prazo</th></tr>`;metas.forEach(m=>{html+=`<tr><td>${m.titulo}</td><td>${m.tipo}</td><td>${Math.min(100,Math.round((m.atual/m.meta)*100))}%</td><td>${m.prazo||"-"}</td></tr>`;});html+=`</table>`;}
...
function PainelMentora({onLogout}){
  ...
  async function cadastrar(){
    ...
    const novo={id:"cli_"+Date.now(),nome,email,senha,movs:[],emocoes:[],crencas:[],metas:[],contas:[],rendimentos:[],ativos:[],rf:[],ativos_usd:[],cotacao_dolar:0,msgs:[{id:1,de:"mentora",texto:`Olá, ${nome}! Bem-vindo(a) à Conduta Rica®!`,hora:"09:00"}],categorias_custom:[],cats_agenda_custom:[],ultimo_mes_acesso:mesAnoAtual(),sequencia_dias:0,ultimo_acesso_dia:""};
  ...
  return(<div style={{fontFamily:"'Segoe UI',sans-serif",background:C.fundo,minHeight:"100vh",color:C.texto}}>
    {editandoCli&&<div style={{position:"fixed",inset:0,background:"#000a",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{background:C.card,borderRadius:14,padding:24,maxWidth:380,width:"90%",border:`2px solid ${C.marsala}`}}>
        ...
        <Inp label="Nome" value={editNome} onChange={setEditNome}/><Inp label="E-mail" value={editEmail} onChange={setEditEmail}/><Inp label="Senha" value={editSenha} onChange={setEditSenha} type="password"/>
        {okEdit&&<div style={{color:C.verde,fontSize:12,marginBottom:8}}>{okEdit}</div>}
        <div style={{display:"flex",gap:10}}><Btn onClick={salvarEdicaoCli} full>Salvar</Btn><Btn onClick={()=>setEditandoCli(null)} outline full>Cancelar</Btn></div>
      </div>
    </div>}
    ...
    <div style={{padding:16,maxWidth:700,margin:"0 auto"}}>
      {aba==="lista"&&<div>
        ...
        {!loading&&clientes.length===0&&<div style={{color:C.textoSuave,textAlign:"center",padding:30}}>Nenhum cliente ainda.</div>}
        {clientes.map(c=>{
          ...
          return(<Card key={c.id}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div><div style={{fontWeight:700,fontSize:14}}>{c.nome}</div><div style={{fontSize:11,color:C.textoSuave,marginBottom:6}}>{c.email}</div>
            ...
            <div style={{textAlign:"right"}}><div style={{fontSize:11,color:C.textoSuave}}>Saldo do mês</div><div style={{fontWeight:700,color:saldoMes>=0?C.verde:C.vermelho,fontSize:13,marginBottom:8}}>{fmt(saldoMes)}</div>
              <div style={{display:"flex",gap:6}}><Btn onClick={()=>setClienteAberto(c)} sm>Abrir</Btn><Btn onClick={()=>abrirEdicao(c)} sm outline>✏</Btn></div>
            </div>
          </div></Card>);
        })}
      </div>}
      {aba==="cadastrar"&&<Card>
        ...
        <Inp label="Nome completo" value={nome} onChange={setNome} placeholder="Nome do mentorado"/><Inp label="E-mail" value={email} onChange={setEmail} placeholder="email@exemplo.com"/><Inp label="Senha" value={senha} onChange={setSenha} type="password" placeholder="Crie uma senha"/>
...
function PainelDados({dados,isMentora,onSalvar,onVoltar,onLogout}){
  ...
  const [cotacaoDolar,setCotacaoDolar]=useState("");const [invTipoUSD,setInvTipoUSD]=useState("stock");
  const [ativoUSDSel,setAtivoUSDSel]=useState("");const [novoAtivoUSD,setNovoAtivoUSD]=useState("");
  const [compQtdUSD,setCompQtdUSD]=useState("");const [compPrecoUSD,setCompPrecoUSD]=useState("");const [compDataUSD,setCompDataUSD]=useState("");
  const [precoAtualUSDSel,setPrecoAtualUSDSel]=useState("");const [precoAtualUSDVal,setPrecoAtualUSDVal]=useState("");

  useEffect(()=>{if(editando)setValEdit({...editando.item});},[editando]);
  useEffect(()=>{msgFim.current?.scrollIntoView({behavior:"smooth"});},[d.msgs]);
  useEffect(()=>{
    if(d.ultimo_mes_acesso&&d.ultimo_mes_acesso!==mesAnoAtual())setModalMigracao(true);
  ...
  const ativos=d.ativos||[];const rf=d.rf||[];const ativos_usd=d.ativos_usd||[];
  const cotacaoDolarSalva=d.cotacao_dolar||0;
  const carteiraCalc=calcCarteira(ativos);
  const carteiraUSDCalc=calcCarteira(ativos_usd);
  const catsCustom=d.categorias_custom||[];const catsAgendaCustom=d.cats_agenda_custom||[];
  const meses=getMesesComLancamentos(movs,rendimentos);
  const totalInvestidoRV=carteiraCalc.reduce((a,x)=>a+(x.valorAtual||x.totalInvestido),0);
  const totalInvestidoRF=rf.reduce((a,x)=>a+(x.valor||0),0);
  const totalCarteiraUSD=carteiraUSDCalc.reduce((a,x)=>a+(x.valorAtual||x.totalInvestido),0)*cotacaoDolarSalva;
  const totalCarteira=totalInvestidoRV+totalInvestidoRF+totalCarteiraUSD;
  const contasVencidas=contas.filter(c=>c.status!=="executada"&&statusConta(c.vencimento)==="vencida").length;
  const emoMovs=movs.filter(m=>m.emocao).map(m=>({nome:m.emocao,tipo:EP.includes(m.emocao)?"positiva":"negativa"}));
  const todasEmo=[...emocoes,...emoMovs];
  const saldoMesAtual=calcSaldoMesComInv(movs,rendimentos,ativos,rf,ativos_usd,cotacaoDolarSalva,mesAnoAtual());
  const pontos=calcPontos(d);const nivel=calcNivel(pontos);const proximoNivel=NIVEIS.find(n=>n.min>pontos);
  const conquistas=calcConquistas(d);const conquistasDesbloqueadas=conquistas.filter(c=>c.desbloqueada);
  ...
  async function migrarSaldo(mesOrigem){
    const s=calcSaldoMes(movs,rendimentos,mesOrigem);
    if(s===0){alert("Saldo zero neste período.");return;}
    const novasMov=[...movs,{id:Date.now(),tipo:"entrada",descricao:"Saldo migrado de "+mesOrigem,valor:Math.abs(s),categoria:"saldo_migrado",emocao:"",data:hoje()}];
  ...
  function addMov(){
    ...
    upd("movs",[...movs,{id:Date.now(),tipo:mTipo,descricao:mDesc,valor:parseFloat(mVal.replace(",",".")),categoria:catFinal,emocao:mEmo||"",data:mData?strParaData(mData).toLocaleDateString("pt-BR"):hoje()}]);
  ...
  function addAtivoECompra(){
    ...
    let novosAtivos;
    if(existente){novosAtivos=ativos.map(a=>a.nome===nomeAtivo?{...a,aportes:[...(a.aportes||[]),{id:Date.now(),qtd:parseFloat(compQtd),preco:parseFloat(compPreco.replace(",",".")),data:dataFmt}]}:a);}
    else{novosAtivos=[...ativos,{id:Date.now(),nome:nomeAtivo,tipo:invTipo,aportes:[{id:Date.now(),qtd:parseFloat(compQtd),preco:parseFloat(compPreco.replace(",",".")),data:dataFmt}],vendas:[],precoAtual:0}];}
    upd("ativos",novosAtivos);setCompQtd("");setCompPreco("");setCompData("");setAtivoSel("");setNovoAtivo("");
  }
  function addVenda(){
    if(!vendaAtivoSel||!vendaQtd||!vendaPreco)return;
    const dataFmt=vendaData?strParaData(vendaData).toLocaleDateString("pt-BR"):hoje();
    upd("ativos",ativos.map(a=>a.nome===vendaAtivoSel?{...a,vendas:[...(a.vendas||[]),{id:Date.now(),qtd:parseFloat(vendaQtd),preco:parseFloat(vendaPreco.replace(",",".")),data:dataFmt}]}:a));
    setVendaQtd("");setVendaPreco("");setVendaData("");setVendaAtivoSel("");
  }
  function atualizarPrecoAtual(){
    if(!precoAtualSel||!precoAtualVal)return;
    upd("ativos",ativos.map(a=>a.nome===precoAtualSel?{...a,precoAtual:parseFloat(precoAtualVal.replace(",","."))}:a));
  ...
  // Investimentos USD
  function addAtivoUSD(){
    if(!compQtdUSD||!compPrecoUSD)return;
    const nomeAtivo=(ativoUSDSel==="novo"?novoAtivoUSD:ativoUSDSel).toUpperCase().trim();
    if(!nomeAtivo)return;
    const dataFmt=compDataUSD?strParaData(compDataUSD).toLocaleDateString("pt-BR"):hoje();
    const existente=ativos_usd.find(a=>a.nome===nomeAtivo);
    let novos;
    if(existente){novos=ativos_usd.map(a=>a.nome===nomeAtivo?{...a,aportes:[...(a.aportes||[]),{id:Date.now(),qtd:parseFloat(compQtdUSD),preco:parseFloat(compPrecoUSD.replace(",",".")),data:dataFmt}]}:a);}
    else{novos=[...ativos_usd,{id:Date.now(),nome:nomeAtivo,tipo:invTipoUSD,aportes:[{id:Date.now(),qtd:parseFloat(compQtdUSD),preco:parseFloat(compPrecoUSD.replace(",",".")),data:dataFmt}],vendas:[],precoAtual:0}];}
    upd("ativos_usd",novos);setCompQtdUSD("");setCompPrecoUSD("");setCompDataUSD("");setAtivoUSDSel("");setNovoAtivoUSD("");
  }
  function atualizarPrecoUSD(){
    if(!precoAtualUSDSel||!precoAtualUSDVal)return;
    upd("ativos_usd",ativos_usd.map(a=>a.nome===precoAtualUSDSel?{...a,precoAtual:parseFloat(precoAtualUSDVal.replace(",","."))}:a));
    setPrecoAtualUSDSel("");setPrecoAtualUSDVal("");
  }
    ...
    return(<div style={{position:"fixed",inset:0,background:"#000b",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
      <div style={{background:C.card,borderRadius:14,padding:20,maxWidth:420,width:"100%",border:`2px solid ${C.marsala}`,maxHeight:"90vh",overflowY:"auto"}}>
        ...
        {lista==="metas"&&<div>
          ...
          <Inp label="Total" value={String(val.meta||"")} onChange={v=>setVal({...val,meta:parseFloat(v)||0})}/>
          <Inp label="Atual" value={String(val.atual||"")} onChange={v=>setVal({...val,atual:parseFloat(v)||0})}/>
  ...
  const opsMeses=[{v:mesAnoAtual(),l:"Mês atual ("+mesAnoAtual()+")"},...meses.filter(m=>m!==mesAnoAtual()).map(m=>({v:m,l:m})),{v:"todos",l:"Todos os períodos"}];
  ...
  function GraficoPizza({dados,titulo}){
    ...
    const total=dados.reduce((a,d)=>a+(d.valor||0),0);if(total===0)return null;
    const CORES_G=["#4A1020","#B8860B","#1B6B3A","#1A4F7A","#8B2252","#2E8B57","#4682B4","#CD853F","#6B8E23","#708090","#C04000","#2F4F4F"];
    let angulo=0;
    const fatias=dados.map((d,i)=>{const pct=(d.valor||0)/total;const ini=angulo;angulo+=pct*2*Math.PI;return{...d,pct,ini,fim:angulo,cor:CORES_G[i%CORES_G.length]};});
    function path(ini,fim){const cx=100,cy=100,r=80;const x1=cx+r*Math.sin(ini),y1=cy-r*Math.cos(ini),x2=cx+r*Math.sin(fim),y2=cy-r*Math.cos(fim);const large=fim-ini>Math.PI?1:0;return `M${cx},${cy} L${x1},${y1} A${r},${r} 0 ${large},1 ${x2},${y2} Z`;}
    return(<div style={{marginBottom:16}}>
      <div style={{fontSize:12,fontWeight:700,color:C.marsala,marginBottom:8}}>{titulo}</div>
      <div style={{display:"flex",gap:16,alignItems:"center",flexWrap:"wrap"}}>
        <svg viewBox="0 0 200 200" style={{width:150,height:150,flexShrink:0}}>{fatias.map((f,i)=><path key={i} d={path(f.ini,f.fim)} fill={f.cor} stroke="#fff" strokeWidth="2"/>)}</svg>
        <div style={{flex:1}}>{fatias.map((f,i)=>(<div key={i} style={{display:"flex",alignItems:"center",gap:6,marginBottom:5}}><div style={{width:12,height:12,borderRadius:3,background:f.cor,flexShrink:0}}/><div style={{fontSize:11}}><div style={{fontWeight:600,color:C.texto}}>{f.categoria||f.nome||"—"}</div><div style={{color:C.textoSuave}}>{fmt(f.valor)} · {(f.pct*100).toFixed(1)}%</div></div></div>))}</div>
      </div>
    </div>);
  }
  ...
  return(<div style={{fontFamily:"'Segoe UI',sans-serif",background:C.fundo,minHeight:"100vh",color:C.texto}}>
    ...
    {modalMigracao&&<div style={{position:"fixed",inset:0,background:"#000a",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
      <div style={{background:C.card,borderRadius:14,padding:24,maxWidth:360,width:"100%",border:`2px solid ${C.marsala}`}}>
        ...
        <div style={{fontSize:13,color:C.textoSuave,marginBottom:6}}>Saldo do mês anterior:</div>
        ...
        <div style={{fontSize:13,marginBottom:16}}>Deseja migrar esse valor como entrada no mês atual?</div>
        <div style={{display:"flex",gap:10}}><Btn onClick={()=>migrarSaldo(d.ultimo_mes_acesso||"")} full>Migrar agora</Btn><Btn onClick={async()=>{await upd("ultimo_mes_acesso",mesAnoAtual());setModalMigracao(false);}} outline full>Depois</Btn></div>
      </div>
    </div>}
    ...
    {modalMigManual&&<div style={{position:"fixed",inset:0,background:"#000a",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
      <div style={{background:C.card,borderRadius:14,padding:24,maxWidth:360,width:"100%",border:`2px solid ${C.marsala}`}}>
        ...
        <Sel label="Mês de origem" value={mesMigrar} onChange={setMesMigrar} options={[{v:"",l:"— Selecione —"},...meses.map(m=>({v:m,l:m}))]}/>
        {mesMigrar&&<div style={{fontSize:13,color:C.textoSuave,marginBottom:12}}>Saldo: <strong style={{color:C.verde}}>{fmt(calcSaldoMes(movs,rendimentos,mesMigrar))}</strong></div>}
        <div style={{display:"flex",gap:10}}><Btn onClick={()=>{if(mesMigrar)migrarSaldo(mesMigrar);}} full>Migrar</Btn><Btn onClick={()=>setModalMigManual(false)} outline full>Cancelar</Btn></div>
      </div>
    </div>}
    ...
    {modalExportar&&<div style={{position:"fixed",inset:0,background:"#000a",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
      <div style={{background:C.card,borderRadius:14,padding:24,maxWidth:360,width:"100%",border:`2px solid ${C.marsala}`}}>
        ...
        <div style={{display:"flex",gap:8,marginTop:16}}><Btn onClick={()=>{exportarPDF(d,modExp);setModalExportar(false);}} style={{flex:1}}>📄 PDF</Btn><Btn onClick={()=>{exportarCSV(d);setModalExportar(false);}} cor={C.azul} style={{flex:1}}>📊 CSV</Btn></div>
    ...
    <div style={{background:`linear-gradient(135deg,${C.marsala},${C.marsalaClaro})`,padding:"14px 20px",borderBottom:`2px solid ${C.dourado}44`}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap",justifyContent:"flex-end"}}>
          ...
          {!isMentora&&<div style={{background:"rgba(0,0,0,0.3)",borderRadius:20,padding:"3px 10px",display:"flex",alignItems:"center",gap:6}}>
            ...
            <span style={{fontSize:10,color:"#f5ede8aa"}}>⭐{pontos}</span>
      ...
      {(()=>{
        ...
        const entradasMes=movs.filter(m=>m.tipo==="entrada"&&mesAnoDeData(m.data)===ma).reduce((a,m)=>a+(m.valor||0),0)+rendimentos.filter(r=>mesAnoDeData(r.data)===ma).reduce((a,r)=>a+(r.valor||0),0);
        const saidasMes=movs.filter(m=>m.tipo==="saida"&&mesAnoDeData(m.data)===ma).reduce((a,m)=>a+(m.valor||0),0);
        return(<div style={{display:"flex",gap:6,marginTop:12,flexWrap:"wrap"}}>
          ...
          <button onClick={()=>setModalMigManual(true)} style={{background:"rgba(0,0,0,0.2)",border:"none",borderRadius:10,padding:"6px 8px",color:C.dourado,fontSize:11,fontWeight:700,cursor:"pointer"}}>🔄</button>
        </div>);
      })()}
    </div>
    ...
    <div style={{display:"flex",overflowX:"auto",background:C.card,borderBottom:`1px solid ${C.marsalaClaro}22`}}>
      {abas.map((a,i)=>(<button key={a} onClick={()=>setAba(i)} style={{padding:"11px 10px",fontSize:11,fontWeight:700,whiteSpace:"nowrap",cursor:"pointer",background:"transparent",border:"none",color:aba===i?C.marsala:C.textoSuave,borderBottom:aba===i?`2px solid ${C.marsala}`:"2px solid transparent"}}>
    ...
    <div style={{padding:16,maxWidth:700,margin:"0 auto"}}>
      ...
      {aba===0&&<div>
        ...
        <div style={{marginBottom:12}}><Sel label="Filtrar por período" value={filtroPeriodo} onChange={setFiltroPeriodo} options={opsMeses}/></div>
        {filtroPeriodo!=="todos"&&<div style={{background:C.cardClaro,borderRadius:8,padding:"8px 12px",marginBottom:12,fontSize:12}}>
          <span style={{color:C.textoSuave}}>Saldo do período: </span><strong style={{color:calcSaldoMes(movs,rendimentos,filtroPeriodo)>=0?C.verde:C.vermelho}}>{fmt(calcSaldoMes(movs,rendimentos,filtroPeriodo))}</strong>
        </div>}
        ...
        {[...movsFiltrados].reverse().map(m=>(<Card key={m.id} style={{padding:"10px 14px"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
            <div style={{flex:1}}><div style={{fontSize:13,fontWeight:600,color:C.texto}}>{m.descricao}</div><div style={{fontSize:11,color:C.textoSuave,marginTop:2}}>{m.data} · {m.categoria}</div>
          ...
          return(<div key={status}><div style={{fontSize:13,fontWeight:700,color:cor,marginBottom:8,marginTop:4}}>{titulo}</div>
            {lista.map(c=>(<Card key={c.id} style={status==="vencida"?{border:`2px solid ${C.vermelho}`,background:C.vermelho+"11"}:status==="executada"?{opacity:0.7}:{}}>
              ...
              {status!=="executada"&&<div style={{display:"flex",gap:6,marginTop:8}}>
                ...
                <Btn sm cor={C.dourado} outline onClick={()=>setEditando({lista:"contas",item:{...c}})}>✏</Btn>
                <Btn sm cor={C.vermelho} outline onClick={()=>setConfirmarExcluir({lista:"contas",id:c.id})}>🗑</Btn>
              </div>}
            </Card>))}
          </div>);
        })}
      ...
      {aba===3&&<div>
        <Card style={{background:`linear-gradient(135deg,${C.marsala}11,${C.azul}11)`}}>
          ...
          <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
            {[{l:"BRL",v:fmt(totalInvestidoRV+totalInvestidoRF),c:C.marsala},{l:"USD (em BRL)",v:fmt(totalCarteiraUSD),c:C.azul},{l:"Total",v:fmt(totalCarteira),c:C.dourado}].map(x=>(
        ...
        <div style={{display:"flex",gap:6,marginBottom:14,flexWrap:"wrap"}}>
          {[["rv","📈 BRL"],["comprar","🛒 Comprar BRL"],["vender","💰 Vender BRL"],["usd","🇺🇸 Dólar"],["rf","🏦 Renda Fixa"]].map(([v,l])=>(
        ...
        {invAba==="rv"&&<div>
          {ativos.length>0&&<Card>
            ...
            <div style={{display:"flex",gap:8,alignItems:"flex-end"}}>
              <div style={{flex:2}}><Sel label="Ativo" value={precoAtualSel} onChange={setPrecoAtualSel} options={[{v:"",l:"— Selecione —"},...ativos.map(a=>({v:a.nome,l:a.nome}))]}/></div>
              <div style={{flex:1}}><Inp label="Preço (R$)" value={precoAtualVal} onChange={setPrecoAtualVal} placeholder="0,00"/></div>
            </div>
            <Btn sm onClick={atualizarPrecoAtual}>Atualizar</Btn>
          </Card>}
          ...
          {carteiraCalc.map(a=>(<Card key={a.id}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
              <div><div style={{fontWeight:800,fontSize:15,color:C.marsala}}>{a.nome}</div><Badge cor={C.azul}>{a.tipo}</Badge></div>
              <div style={{textAlign:"right"}}>{a.precoAtual>0&&<><div style={{fontSize:12,color:C.textoSuave}}>Atual: <strong>{fmt(a.precoAtual)}</strong></div><div style={{fontSize:13,fontWeight:700,color:a.pct>=0?C.verde:C.vermelho}}>{fmtPct(a.pct)}</div></>}</div>
            </div>
            ...
            {(a.aportes||[]).length>0&&<div style={{marginTop:8}}>
              ...
              {(a.aportes||[]).map(ap=>(<div key={ap.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",fontSize:11,padding:"4px 0",borderBottom:`1px solid ${C.cardClaro}`}}>
                <span style={{color:C.textoSuave,flex:1}}>{ap.data} — {Number(ap.qtd||0).toLocaleString("pt-BR",{maximumFractionDigits:4})} × {fmt(ap.preco||0)} = {fmt((ap.qtd||0)*(ap.preco||0))}</span>
                <div style={{display:"flex",gap:4,marginLeft:8}}>
                  <button onClick={()=>{const nq=window.prompt("Nova quantidade:",ap.qtd);const np=window.prompt("Novo preço:",ap.preco);if(nq!==null&&np!==null)upd("ativos",ativos.map(x=>x.nome===a.nome?{...x,aportes:(x.aportes||[]).map(p=>p.id===ap.id?{...p,qtd:parseFloat(nq)||ap.qtd,preco:parseFloat(np.replace(",","."))||ap.preco}:p)}:x));}} style={{background:"transparent",border:`1px solid ${C.dourado}`,color:C.dourado,borderRadius:6,padding:"2px 6px",fontSize:10,cursor:"pointer"}}>✏</button>
                  <button onClick={()=>{if(window.confirm("Excluir?"))upd("ativos",ativos.map(x=>x.nome===a.nome?{...x,aportes:(x.aportes||[]).filter(p=>p.id!==ap.id)}:x));}} style={{background:"transparent",border:`1px solid ${C.vermelho}`,color:C.vermelho,borderRadius:6,padding:"2px 6px",fontSize:10,cursor:"pointer"}}>🗑</button>
                </div>
              </div>))}
            </div>}
            {(a.vendas||[]).length>0&&<div style={{marginTop:8}}><div style={{fontSize:11,color:C.textoSuave,marginBottom:4,fontWeight:600}}>Vendas:</div>{(a.vendas||[]).map(v=>(<div key={v.id} style={{fontSize:11,color:C.verde,padding:"2px 0"}}>{v.data} — {v.qtd} × {fmt(v.preco)} = {fmt((v.qtd||0)*(v.preco||0))}</div>))}</div>}
          </Card>))}
        </div>}

        {invAba==="comprar"&&<Card>
          <div style={{fontSize:14,fontWeight:700,color:C.marsala,marginBottom:12}}>🛒 Registrar Compra (BRL)</div>
          ...
          {ativoSel==="novo"&&<div><Inp label="Nome do ativo" value={novoAtivo} onChange={setNovoAtivo} placeholder="Ex: HGLG11"/><Sel label="Tipo" value={invTipo} onChange={setInvTipo} options={TIPOS_INV}/></div>}
        ...
        {invAba==="vender"&&<Card>
          <div style={{fontSize:14,fontWeight:700,color:C.marsala,marginBottom:12}}>💰 Registrar Venda (BRL)</div>
          {carteiraCalc.filter(a=>a.totalCotas>0).length===0?<div style={{color:C.textoSuave,fontSize:13}}>Nenhum ativo disponível para venda.</div>:<div>
            ...
            <Inp label="Quantidade" value={vendaQtd} onChange={setVendaQtd} type="number" step="0.0001" placeholder="0"/>
            <Inp label="Preço de venda (R$)" value={vendaPreco} onChange={setVendaPreco} placeholder="0,00"/>
            {vendaQtd&&vendaPreco&&<div style={{fontSize:13,marginBottom:12}}><span style={{color:C.textoSuave}}>Total: </span><strong style={{color:C.verde}}>{fmt(parseFloat(vendaQtd||0)*parseFloat(vendaPreco.replace(",",".")||0))}</strong></div>}
        ...
        {invAba==="usd"&&<div>
          <Card style={{background:C.azul+"11",border:`1px solid ${C.azul}33`}}>
            <div style={{fontSize:13,fontWeight:700,color:C.azul,marginBottom:10}}>🇺🇸 Cotação do Dólar</div>
            <div style={{display:"flex",gap:8,alignItems:"flex-end"}}>
              <div style={{flex:1}}><Inp label="Cotação atual (R$)" value={cotacaoDolar} onChange={setCotacaoDolar} placeholder="Ex: 5,80"/></div>
              <Btn sm cor={C.azul} onClick={()=>{if(cotacaoDolar){upd("cotacao_dolar",parseFloat(cotacaoDolar.replace(",",".")));setCotacaoDolar("");}}}>Salvar</Btn>
            </div>
            {cotacaoDolarSalva>0&&<div style={{fontSize:11,color:C.textoSuave,marginTop:4}}>Cotação salva: <strong style={{color:C.azul}}>{fmt(cotacaoDolarSalva)}</strong></div>}
          </Card>
          <Card>
            <div style={{fontSize:14,fontWeight:700,color:C.marsala,marginBottom:12}}>🛒 Comprar Ativo em Dólar</div>
            <Sel label="Ativo" value={ativoUSDSel} onChange={setAtivoUSDSel} options={[{v:"",l:"— Selecione —"},...ativos_usd.map(a=>({v:a.nome,l:a.nome})),{v:"novo",l:"+ Novo ativo"}]}/>
            {ativoUSDSel==="novo"&&<div><Inp label="Nome do ativo (ex: AAPL)" value={novoAtivoUSD} onChange={setNovoAtivoUSD} placeholder="Ticker"/><Sel label="Tipo" value={invTipoUSD} onChange={setInvTipoUSD} options={TIPOS_INV_USD}/></div>}
            <Inp label="Preço por cota (USD $)" value={compPrecoUSD} onChange={setCompPrecoUSD} placeholder="0.00"/>
            <Inp label="Quantidade" value={compQtdUSD} onChange={setCompQtdUSD} type="number" step="0.0001" placeholder="0"/>
            {compQtdUSD&&compPrecoUSD&&<div style={{fontSize:12,color:C.azul,marginBottom:8,fontWeight:600}}>
              Total: $ {(parseFloat(compQtdUSD||0)*parseFloat(compPrecoUSD.replace(",",".")||0)).toFixed(2)}
              {cotacaoDolarSalva>0&&<span style={{color:C.textoSuave,fontWeight:400}}> = {fmt(parseFloat(compQtdUSD||0)*parseFloat(compPrecoUSD.replace(",",".")||0)*cotacaoDolarSalva)}</span>}
            </div>}
            <Inp label="Data (em branco = hoje)" value={compDataUSD} onChange={setCompDataUSD} type="date"/>
            <Btn onClick={addAtivoUSD}>Registrar Compra</Btn>
          </Card>
          {carteiraUSDCalc.length>0&&<Card>
            <div style={{fontSize:13,fontWeight:700,color:C.marsala,marginBottom:10}}>Atualizar Preço Atual (USD)</div>
            <div style={{display:"flex",gap:8,alignItems:"flex-end"}}>
              <div style={{flex:2}}><Sel label="Ativo" value={precoAtualUSDSel} onChange={setPrecoAtualUSDSel} options={[{v:"",l:"— Selecione —"},...ativos_usd.map(a=>({v:a.nome,l:a.nome}))]}/></div>
              <div style={{flex:1}}><Inp label="Preço (USD)" value={precoAtualUSDVal} onChange={setPrecoAtualUSDVal} placeholder="0.00"/></div>
            </div>
            <Btn sm onClick={atualizarPrecoUSD}>Atualizar</Btn>
          </Card>}
          {carteiraUSDCalc.length===0&&<div style={{color:C.textoSuave,textAlign:"center",padding:20,fontSize:13}}>Nenhum ativo em dólar ainda.</div>}
          {carteiraUSDCalc.map(a=>(<Card key={a.id}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
              <div><div style={{fontWeight:800,fontSize:15,color:C.azul}}>{a.nome} 🇺🇸</div><Badge cor={C.azul}>{a.tipo}</Badge></div>
              <div style={{textAlign:"right"}}>{a.precoAtual>0&&<><div style={{fontSize:12,color:C.textoSuave}}>Atual: <strong>$ {a.precoAtual.toFixed(2)}</strong></div><div style={{fontSize:13,fontWeight:700,color:a.pct>=0?C.verde:C.vermelho}}>{fmtPct(a.pct)}</div></>}</div>
            </div>
            <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:8}}>
              {[{l:"Cotas",v:Number(a.totalCotas).toLocaleString("pt-BR",{maximumFractionDigits:4})},{l:"PM (USD)",v:"$ "+a.precoMedio.toFixed(2)},{l:"Invest. (USD)",v:"$ "+a.totalInvestido.toFixed(2)},{l:"Val. BRL",v:a.precoAtual>0&&cotacaoDolarSalva>0?fmt(a.valorAtual*cotacaoDolarSalva):"—"}].map(x=>(
                <div key={x.l} style={{background:C.cardClaro,borderRadius:8,padding:"6px 10px",flex:1,minWidth:70,textAlign:"center"}}>
                  <div style={{fontSize:10,color:C.textoSuave}}>{x.l}</div>
                  <div style={{fontSize:12,fontWeight:700,color:C.texto}}>{x.v}</div>
                </div>
            ...
            {(a.aportes||[]).length>0&&<div style={{marginTop:4}}>
              <div style={{fontSize:11,color:C.textoSuave,marginBottom:4,fontWeight:600}}>Aportes:</div>
              {(a.aportes||[]).map(ap=>(<div key={ap.id} style={{fontSize:11,color:C.textoSuave,padding:"3px 0",borderBottom:`1px solid ${C.cardClaro}`}}>
                {ap.data} — {Number(ap.qtd||0).toLocaleString("pt-BR",{maximumFractionDigits:4})} × $ {(ap.preco||0).toFixed(2)} = $ {((ap.qtd||0)*(ap.preco||0)).toFixed(2)}
                {cotacaoDolarSalva>0&&<span style={{color:C.azul}}> ({fmt((ap.qtd||0)*(ap.preco||0)*cotacaoDolarSalva)})</span>}
              </div>))}
            </div>}
          </Card>))}
        </div>}
        ...
        {invAba==="rf"&&<div>
          ...
          {rf.map(r=>(<Card key={r.id} style={{padding:"10px 14px"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div><div style={{fontSize:13,fontWeight:600,color:C.texto}}>{r.descricao}</div><div style={{fontSize:11,color:C.textoSuave}}>{r.data} · {r.taxa||"—"} · Vence: {r.vencimento||"—"}</div></div>
              <div style={{fontWeight:700,color:C.verde}}>{fmt(r.valor)}</div>
            </div>
            <div style={{marginTop:8}}><Btn sm cor={C.vermelho} outline onClick={()=>upd("rf",rf.filter(x=>x.id!==r.id))}>🗑 Excluir</Btn></div>
          </Card>))}
        </div>}
      </div>}
      ...
      {aba===6&&<div>
        <Card>
          ...
          <div style={{display:"flex",gap:8}}><div style={{flex:1}}><Inp label="Total" value={metaVal} onChange={setMetaVal} placeholder="100"/></div><div style={{flex:1}}><Inp label="Atual" value={metaAt} onChange={setMetaAt} placeholder="0"/></div></div>
      ...
      {aba===7&&<Card>
        ...
        <div style={{fontSize:13,fontWeight:700,color:C.marsala,marginBottom:10}}>📊 Gráficos do Mês</div>
        {(()=>{
          const ma=mesAnoAtual();
          const movMes=movs.filter(m=>mesAnoDeData(m.data)===ma&&m.tipo!=="investimento");
          const entMes=movMes.filter(m=>m.tipo==="entrada");const saiMes=movMes.filter(m=>m.tipo==="saida");
          function agrupar(lista){const m={};lista.forEach(x=>{const k=x.categoria||"outros";m[k]=(m[k]||0)+(x.valor||0);});return Object.entries(m).map(([categoria,valor])=>({categoria,valor})).sort((a,b)=>b.valor-a.valor);}
          return(<div><GraficoPizza dados={agrupar(entMes)} titulo="Entradas por Categoria"/><GraficoPizza dados={agrupar(saiMes)} titulo="Saídas por Categoria"/></div>);
        })()}

        {rendimentos.length>0&&<div style={{marginBottom:16}}>
          <div style={{fontSize:12,fontWeight:700,color:C.marsala,marginBottom:8}}>💰 Rendimentos por Tipo</div>
          {(()=>{
            const ag={};rendimentos.forEach(r=>{ag[r.tipo]=(ag[r.tipo]||0)+(r.valor||0);});
            const dados=Object.entries(ag).map(([categoria,valor])=>({categoria,valor})).sort((a,b)=>b.valor-a.valor);
            const total=dados.reduce((a,d)=>a+(d.valor||0),0);
            const CORES_R=["#1B6B3A","#B8860B","#1A4F7A","#4A1020","#8B2252","#2E8B57"];
            let angulo=0;
            const fatias=dados.map((d,i)=>{const pct=(d.valor||0)/total;const ini=angulo;angulo+=pct*2*Math.PI;return{...d,pct,ini,fim:angulo,cor:CORES_R[i%CORES_R.length]};});
            function path(ini,fim){const cx=100,cy=100,r=80;const x1=cx+r*Math.sin(ini),y1=cy-r*Math.cos(ini),x2=cx+r*Math.sin(fim),y2=cy-r*Math.cos(fim);const large=fim-ini>Math.PI?1:0;return `M${cx},${cy} L${x1},${y1} A${r},${r} 0 ${large},1 ${x2},${y2} Z`;}
            return(<div>
              <div style={{display:"flex",gap:16,alignItems:"center",flexWrap:"wrap",marginBottom:12}}>
                <svg viewBox="0 0 200 200" style={{width:150,height:150,flexShrink:0}}>{fatias.map((f,i)=><path key={i} d={path(f.ini,f.fim)} fill={f.cor} stroke="#fff" strokeWidth="2"/>)}</svg>
                <div style={{flex:1}}>{fatias.map((f,i)=>(<div key={i} style={{display:"flex",alignItems:"center",gap:6,marginBottom:5}}><div style={{width:12,height:12,borderRadius:3,background:f.cor,flexShrink:0}}/><div style={{fontSize:11}}><div style={{fontWeight:600,color:C.texto}}>{f.categoria}</div><div style={{color:C.textoSuave}}>{fmt(f.valor)} · {(f.pct*100).toFixed(1)}%</div></div></div>))}<div style={{marginTop:8,fontSize:12,fontWeight:700,color:C.verde}}>Total: {fmt(total)}</div></div>
              </div>
              {[...rendimentos].sort((a,b)=>b.data?.localeCompare(a.data||"")||0).map(r=>(<div key={r.id} style={{display:"flex",justifyContent:"space-between",padding:"5px 0",borderBottom:`1px solid ${C.cardClaro}`,fontSize:12}}><span style={{color:C.textoSuave}}>{r.data} · {r.descricao}</span><span style={{fontWeight:600,color:C.verde}}>+{fmt(r.valor)}</span></div>))}
            </div>);
          })()}
        </div>}

        {(carteiraCalc.length>0||carteiraUSDCalc.length>0||rf.length>0)&&<div style={{marginBottom:16}}>
          <div style={{fontSize:12,fontWeight:700,color:C.marsala,marginBottom:8}}>📊 Carteira por Ativo</div>
          {(()=>{
            const CORES_I=["#4A1020","#B8860B","#1B6B3A","#1A4F7A","#8B2252","#2E8B57","#4682B4","#CD853F","#6B8E23","#708090"];
            const dados=[...carteiraCalc.map(a=>({categoria:a.nome,valor:a.valorAtual||a.totalInvestido})),...carteiraUSDCalc.map(a=>({categoria:a.nome+" 🇺🇸",valor:(a.valorAtual||a.totalInvestido)*cotacaoDolarSalva})),...(rf.length>0?[{categoria:"Renda Fixa",valor:totalInvestidoRF}]:[])].filter(d=>(d.valor||0)>0).sort((a,b)=>b.valor-a.valor);
            const total=dados.reduce((a,d)=>a+(d.valor||0),0);if(total===0)return null;
            let angulo=0;
            const fatias=dados.map((d,i)=>{const pct=(d.valor||0)/total;const ini=angulo;angulo+=pct*2*Math.PI;return{...d,pct,ini,fim:angulo,cor:CORES_I[i%CORES_I.length]};});
            function path(ini,fim){const cx=100,cy=100,r=80;const x1=cx+r*Math.sin(ini),y1=cy-r*Math.cos(ini),x2=cx+r*Math.sin(fim),y2=cy-r*Math.cos(fim);const large=fim-ini>Math.PI?1:0;return `M${cx},${cy} L${x1},${y1} A${r},${r} 0 ${large},1 ${x2},${y2} Z`;}
            return(<div style={{display:"flex",gap:16,alignItems:"center",flexWrap:"wrap"}}>
              <svg viewBox="0 0 200 200" style={{width:150,height:150,flexShrink:0}}>{fatias.map((f,i)=><path key={i} d={path(f.ini,f.fim)} fill={f.cor} stroke="#fff" strokeWidth="2"/>)}</svg>
              <div style={{flex:1}}>{fatias.map((f,i)=>(<div key={i} style={{display:"flex",alignItems:"center",gap:6,marginBottom:5}}><div style={{width:12,height:12,borderRadius:3,background:f.cor,flexShrink:0}}/><div style={{fontSize:11}}><div style={{fontWeight:600,color:C.texto}}>{f.categoria}</div><div style={{color:C.textoSuave}}>{fmt(f.valor)} · {(f.pct*100).toFixed(1)}%</div></div></div>))}<div style={{marginTop:8,fontSize:12,fontWeight:700,color:C.marsala}}>Total: {fmt(total)}</div></div>
            </div>);
          })()}
        </div>}

        <div style={{fontSize:13,fontWeight:700,color:C.marsala,marginTop:8,marginBottom:10}}>💰 Extrato por Mês</div>
        ...
        {meses.map(ma=>{
          ...
          return(<div key={ma} style={{marginBottom:14}}>
            ...
            {movMes.map(m=>(<div key={m.id} style={{display:"flex",justifyContent:"space-between",padding:"4px 12px",fontSize:12}}><span style={{color:C.textoSuave}}>{m.descricao} · {m.categoria}{m.emocao?" · "+m.emocao:""}</span><span style={{fontWeight:600,color:m.tipo==="entrada"?C.verde:C.vermelho}}>{m.tipo==="saida"?"-":""}{fmt(m.valor)}</span></div>))}
            {rendMes.map(r=>(<div key={r.id} style={{display:"flex",justifyContent:"space-between",padding:"4px 12px",fontSize:12}}><span style={{color:C.textoSuave}}>{r.descricao} · rendimento</span><span style={{fontWeight:600,color:C.verde}}>+{fmt(r.valor)}</span></div>))}
          </div>);
        })}

        <div style={{fontSize:13,fontWeight:700,color:C.marsala,marginTop:8,marginBottom:8}}>😊 Mapa Emocional</div>
        ...
        {crencas.length===0?<div style={{color:C.textoSuave,fontSize:12}}>Nenhuma crença mapeada.</div>:<div><div style={{fontSize:12,color:C.textoSuave,marginBottom:6}}>Total: {crencas.length} · Ativas: {crencas.filter(c=>c.status==="ativa").length} · Ressignificadas: {crencas.filter(c=>c.status==="ressignificada").length}</div>{crencas.map(c=>(<div key={c.id} style={{padding:"6px 10px",borderLeft:`3px solid ${c.status==="ressignificada"?C.verde:C.vermelho}`,marginBottom:6,background:C.cardClaro,borderRadius:"0 6px 6px 0"}}><div style={{fontSize:12,fontWeight:600,color:C.texto}}>{c.crenca}</div><div style={{fontSize:11,color:C.textoSuave}}>{c.status==="ressignificada"?"✅ Ressignificada":"⚠️ Em processo"}</div></div>))}</div>}
      ...
      {aba===8&&<div>
        <Card style={{background:`linear-gradient(135deg,${nivel.cor}22,${nivel.cor}11)`}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
            <div><div style={{fontSize:11,color:C.textoSuave}}>Nível atual</div><div style={{fontSize:22,fontWeight:900,color:nivel.cor}}>{nivel.nome}</div><div style={{fontSize:12,color:C.textoSuave}}>⭐ {pontos} pontos</div></div>
            <div style={{fontSize:48}}>{nivel.nivel===1?"🌱":nivel.nivel===2?"👁️":nivel.nivel===3?"💡":nivel.nivel===4?"⚡":"🏆"}</div>
          </div>
          {proximoNivel&&<div>
            <div style={{display:"flex",justifyContent:"space-between",fontSize:11,color:C.textoSuave,marginBottom:4}}><span>Próximo: {proximoNivel.nome}</span><span>{pontos}/{proximoNivel.min} pts</span></div>
            <div style={{background:C.cardClaro,borderRadius:20,height:8,overflow:"hidden"}}><div style={{width:`${Math.min(100,(pontos/proximoNivel.min)*100)}%`,background:`linear-gradient(90deg,${nivel.cor},${proximoNivel.cor})`,height:"100%",borderRadius:20}}/></div>
          </div>}
          {!proximoNivel&&<div style={{fontSize:13,fontWeight:700,color:nivel.cor,textAlign:"center",marginTop:8}}>🏆 Nível máximo atingido!</div>}
        </Card>
        <Card>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div><div style={{fontSize:14,fontWeight:700,color:C.marsala}}>🔥 Sequência de Dias</div><div style={{fontSize:12,color:C.textoSuave,marginTop:2}}>Dias consecutivos no app</div></div>
            <div style={{fontSize:36,fontWeight:900,color:sequenciaDias>=7?"#e07b00":C.textoSuave}}>{sequenciaDias}</div>
          </div>
          <div style={{display:"flex",gap:4,marginTop:10}}>
            {[7,14,30].map(meta=>(<div key={meta} style={{flex:1,textAlign:"center",padding:"6px",borderRadius:8,background:sequenciaDias>=meta?C.marsala:C.cardClaro}}><div style={{fontSize:11,fontWeight:700,color:sequenciaDias>=meta?"#fff":C.textoSuave}}>{meta} dias</div><div style={{fontSize:9,color:sequenciaDias>=meta?"#f5ede8aa":C.textoSuave}}>{sequenciaDias>=meta?"✅":"🔒"}</div></div>))}
          </div>
        </Card>
        ...
        {CONQUISTAS.map(c=>(<div key={c.id} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 14px",background:c.desbloqueada?C.card:C.cardClaro,borderRadius:12,marginBottom:10,border:`1px solid ${c.desbloqueada?C.marsala+"33":C.cardClaro}`,opacity:c.desbloqueada?1:0.6}}>
          <div style={{fontSize:28,filter:c.desbloqueada?"none":"grayscale(1)"}}>{c.icone}</div>
          <div style={{flex:1}}><div style={{fontSize:13,fontWeight:700,color:c.desbloqueada?C.marsala:C.textoSuave}}>{c.nome}</div><div style={{fontSize:11,color:C.textoSuave}}>{c.desc}</div></div>
          <div style={{textAlign:"right"}}><div style={{fontSize:12,fontWeight:700,color:c.desbloqueada?C.dourado:C.textoSuave}}>+{c.pts}pts</div><div style={{fontSize:10,color:c.desbloqueada?C.verde:C.textoSuave}}>{c.desbloqueada?"✅":"🔒"}</div></div>
        </div>))}
      </div>}
