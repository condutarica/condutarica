...
function Login({onLogin}){
  ...
  return(<div style={{minHeight:"100vh",background:CORES.fundo,display:"flex",alignItems:"center",justifyContent:"center",padding:20,fontFamily:"'Segoe UI',sans-serif"}}>
    <div style={{width:"100%",maxWidth:380}}>
      <div style={{textAlign:"center",marginBottom:32}}><div style={{fontSize:28,fontWeight:900,color:CORES.marsala,letterSpacing:2}}>CONDUTA RICA®</div><div style={{fontSize:13,color:CORES.textoSuave,marginTop:4}}>Rastreador Comportamental-Financeiro</div></div>
      <Card>
        <div style={{fontSize:15,fontWeight:700,color:CORES.marsala,marginBottom:16,textAlign:"center"}}>Acesse sua conta</div>
        <Inp label="E-mail" value={email} onChange={setEmail} placeholder="seu@email.com"/>
        <div style={{marginBottom:12}}><label style={{display:"block",fontSize:12,color:CORES.dourado,marginBottom:4,fontWeight:600}}>Senha</label><input type="password" value={senha} onChange={e=>setSenha(e.target.value)} onKeyDown={e=>e.key==="Enter"&&entrar()} placeholder="••••••••" style={{width:"100%",background:CORES.cardClaro,border:`1px solid ${CORES.marsalaClaro}55`,borderRadius:8,padding:"8px 12px",color:CORES.texto,fontSize:14,boxSizing:"border-box",outline:"none"}}/></div>
        {erro&&<div style={{color:CORES.vermelho,fontSize:12,marginBottom:10,textAlign:"center"}}>{erro}</div>}
        <button onClick={entrar} disabled={loading} style={{width:"100%",background:CORES.marsala,color:"#fff",border:`2px solid ${CORES.marsala}`,borderRadius:8,padding:"10px",fontWeight:700,fontSize:14,cursor:"pointer"}}>{loading?"Entrando...":"Entrar"}</button>
      </Card>
...
function PainelMentora({onLogout}){
  ...
  return(<div style={{fontFamily:"'Segoe UI',sans-serif",background:CORES.fundo,minHeight:"100vh",color:CORES.texto}}>
    ...
    <div style={{display:"flex",background:CORES.card,borderBottom:`1px solid ${CORES.marsalaClaro}33`}}>
      {[["lista","Meus Clientes"],["cadastrar","Cadastrar Cliente"]].map(([v,l])=>(<button key={v} onClick={()=>setAba(v)} style={{padding:"12px 20px",fontSize:12,fontWeight:700,cursor:"pointer",background:"transparent",border:"none",color:aba===v?CORES.marsala:CORES.textoSuave,borderBottom:aba===v?`2px solid ${CORES.marsala}`:"2px solid transparent"}}>{l}</button>))}
    </div>
    <div style={{padding:16,maxWidth:700,margin:"0 auto"}}>
      {aba==="lista"&&<div>
        <div style={{fontSize:14,fontWeight:700,color:CORES.marsala,marginBottom:12}}>Clientes ({clientes.length})</div>
      ...
      {aba==="cadastrar"&&<Card>
        <div style={{fontSize:14,fontWeight:700,color:CORES.marsala,marginBottom:14}}>Novo cliente</div>
...
function PainelDados({dados,isMentora,onSalvar,onVoltar,onLogout}){
  ...
  function ModalEdicao(){
    ...
    return(<div style={{position:"fixed",inset:0,background:"#000b",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
      <div style={{background:CORES.card,borderRadius:14,padding:20,maxWidth:420,width:"100%",border:`2px solid ${CORES.marsala}`,maxHeight:"90vh",overflowY:"auto"}}>
        <div style={{fontSize:15,fontWeight:700,color:CORES.marsala,marginBottom:14}}>✏ Editar registro</div>
        {lista==="movs"&&<div>
  ...
  return(<div style={{fontFamily:"'Segoe UI',sans-serif",background:CORES.fundo,minHeight:"100vh",color:CORES.texto}}>
    ...
    <div style={{background:`linear-gradient(135deg,${CORES.marsala},${CORES.marsalaClaro})`,padding:"14px 20px",borderBottom:`2px solid ${CORES.dourado}44`}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <div><div style={{fontSize:15,fontWeight:800,color:CORES.dourado}}>CONDUTA RICA®</div><div style={{fontSize:11,color:"#f5ede8"}}>{d.nome}</div></div>
        </div>
      ...
      <div style={{display:"flex",gap:8,marginTop:12,flexWrap:"wrap"}}>
        {[{l:"Entradas",v:fmt(entradas+totalRend),c:"#4caf7d"},{l:"Saídas",v:fmt(saidas),c:"#e05c5c"},{l:"Investido",v:fmt(inv),c:"#90caf9"},{l:"Saldo",v:fmt(saldo),c:CORES.dourado}].map(x=>(
          <div key={x.l} style={{background:"rgba(0,0,0,0.2)",borderRadius:10,padding:"6px 12px",flex:1,minWidth:60,textAlign:"center"}}>
    ...
    <div style={{display:"flex",overflowX:"auto",background:CORES.card,borderBottom:`1px solid ${CORES.marsalaClaro}22`}}>
      {abas.map((a,i)=>(<button key={a} onClick={()=>setAba(i)} style={{padding:"11px 12px",fontSize:11,fontWeight:700,whiteSpace:"nowrap",cursor:"pointer",background:"transparent",border:"none",color:aba===i?CORES.marsala:CORES.textoSuave,borderBottom:aba===i?`2px solid ${CORES.marsala}`:"2px solid transparent"}}>
    ...
    <div style={{padding:16,maxWidth:700,margin:"0 auto"}}>
      ...
      {aba===0&&<div>
        <Card>
          <div style={{fontSize:14,fontWeight:700,color:CORES.marsala,marginBottom:12}}>Nova Movimentação</div>
      ...
      {aba===2&&<div>
        <Card>
          <div style={{fontSize:14,fontWeight:700,color:CORES.marsala,marginBottom:12}}>Registrar Rendimento</div>
      ...
      {aba===3&&<div>
        <Card>
          <div style={{fontSize:14,fontWeight:700,color:CORES.marsala,marginBottom:12}}>Registrar Investimento</div>
      ...
      {aba===5&&<div>
        ...
        {crencas.map(c=>(<Card key={c.id}>
          ...
          {isMentora&&<div style={{marginBottom:8}}><label style={{fontSize:11,color:CORES.dourado,fontWeight:600,display:"block",marginBottom:4}}>Nota da Mentora:</label><textarea value={c.notaMentora} onChange={e=>notaMentora(c.id,e.target.value)} rows={2} placeholder="Observação clínica..." style={{width:"100%",background:CORES.cardClaro,border:`1px solid ${CORES.marsalaClaro}55`,borderRadius:8,padding:"6px 10px",color:CORES.texto,fontSize:12,boxSizing:"border-box",resize:"vertical"}}/></div>}
          {!isMentora&&c.notaMentora&&<div style={{fontSize:12,color:CORES.marsala,background:CORES.cardClaro,borderRadius:8,padding:"6px 10px",marginBottom:8,borderLeft:`3px solid ${CORES.marsala}`}}><span style={{fontWeight:600}}>Feedback da mentora: </span>{c.notaMentora}</div>}
          <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
            <Btn sm cor={c.status==="ressignificada"?CORES.verde:CORES.marsala} outline onClick={()=>toggleCrencaStatus(c.id)}>{c.status==="ressignificada"?"Reativar":"Marcar Ressignificada"}</Btn>
      ...
      {aba===6&&<div>
        <Card>
          <div style={{fontSize:14,fontWeight:700,color:CORES.marsala,marginBottom:12}}>Nova Meta</div>
        ...
        {metas.map(m=>(<Card key={m.id}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}><div style={{fontWeight:700,color:CORES.texto}}>{m.titulo}</div><Badge cor={m.tipo==="financeira"?CORES.marsala:m.tipo==="comportamental"?CORES.azul:CORES.verde}>{m.tipo}</Badge></div>
          ...
          <div style={{display:"flex",gap:8,marginTop:10,alignItems:"center"}}><span style={{fontSize:12,color:CORES.textoSuave}}>Atualizar:</span><input type="number" value={m.atual} onChange={e=>updMeta(m.id,"atual",e.target.value)} style={{width:80,background:CORES.cardClaro,border:`1px solid ${CORES.marsalaClaro}55`,borderRadius:6,padding:"4px 8px",color:CORES.texto,fontSize:13}}/><span style={{fontSize:12,color:CORES.textoSuave}}>de {m.meta}</span></div>
          <AcoesCard lista="metas" item={m}/>
        </Card>))}
      </div>}

      {aba===7&&<Card>
        <div style={{fontSize:15,fontWeight:800,color:CORES.marsala,marginBottom:2}}>Relatório — {d.nome}</div>
        ...
        <div style={{fontSize:13,fontWeight:700,color:CORES.marsala,marginBottom:8}}>💰 Resumo Financeiro</div>
        {[{l:"Entradas",v:fmt(entradas),c:CORES.verde},{l:"Rendimentos",v:fmt(totalRend),c:CORES.verde},{l:"Saídas",v:fmt(saidas),c:CORES.vermelho},{l:"Total Investido",v:fmt(inv),c:CORES.azul},{l:"Saldo Líquido",v:fmt(saldo),c:saldo>=0?CORES.verde:CORES.vermelho}].map(x=>(<div key={x.l} style={{display:"flex",justifyContent:"space-between",padding:"6px 0",borderBottom:`1px solid ${CORES.cardClaro}`}}><span style={{fontSize:13,color:CORES.textoSuave}}>{x.l}</span><span style={{fontSize:13,fontWeight:700,color:x.c}}>{x.v}</span></div>))}
      ...
      {aba===8&&<Card style={{padding:0,overflow:"hidden"}}>
        <div style={{background:CORES.marsala,padding:"10px 16px"}}><div style={{fontWeight:700,color:CORES.dourado,fontSize:13}}>Canal de Comunicação</div><div style={{fontSize:11,color:"#f5ede8"}}>Enviando como: {isMentora?"Wélica Amaro (Mentora)":d.nome}</div></div>
        <div style={{height:320,overflowY:"auto",padding:12,display:"flex",flexDirection:"column",gap:8}}>
          {(d.msgs||[]).map(m=>(<div key={m.id} style={{display:"flex",justifyContent:m.de==="mentora"?"flex-end":"flex-start"}}><div style={{maxWidth:"78%",padding:"8px 12px",borderRadius:m.de==="mentora"?"14px 14px 4px 14px":"14px 14px 14px 4px",background:m.de==="mentora"?CORES.marsala:CORES.cardClaro,border:`1px solid ${m.de==="mentora"?CORES.marsala:CORES.cardClaro}`}}><div style={{fontSize:13,color:m.de==="mentora"?"#fff":CORES.texto}}>{m.texto}</div><div style={{fontSize:10,color:m.de==="mentora"?"#f5ede8aa":CORES.textoSuave,marginTop:4,textAlign:"right"}}>{m.hora} · {m.de==="mentora"?"Wélica Amaro":d.nome}</div></div></div>))}
          <div ref={msgFim}/>
        </div>
        <div style={{padding:"10px 12px",borderTop:`1px solid ${CORES.cardClaro}`,display:"flex",gap:8}}>
          <input value={msg} onChange={e=>setMsg(e.target.value)} onKeyDown={e=>e.key==="Enter"&&enviar()} placeholder="Digite sua mensagem..." style={{flex:1,background:CORES.cardClaro,border:`1px solid ${CORES.marsalaClaro}33`,borderRadius:20,padding:"8px 14px",color:CORES.texto,fontSize:13,outline:"none"}}/>
          <button onClick={enviar} style={{background:CORES.marsala,border:"none",borderRadius:20,padding:"8px 16px",color:"#fff",fontWeight:700,cursor:"pointer"}}>Enviar</button>
        </div>
      </Card>}

      {aba===9&&<Card>
        <div style={{fontSize:14,fontWeight:700,color:CORES.marsala,marginBottom:16}}>👤 Meu Perfil</div>
        ...
        <div style={{fontSize:14,fontWeight:700,color:CORES.marsala,marginBottom:12}}>🔒 Alterar Senha</div>
    ...
    <div style={{textAlign:"center",padding:14,fontSize:11,color:CORES.textoSuave,borderTop:`1px solid ${CORES.cardClaro}`}}>Conduta Rica® · Wélica Amaro — CRP 09/12387</div>
  </div>);
}
